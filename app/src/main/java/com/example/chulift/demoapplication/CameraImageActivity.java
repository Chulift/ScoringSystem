...
public class CameraImageActivity extends AppCompatActivity {
    ...
    String url = Config.serverUrl + Config.projectName + "uploadAnswerSheet.php";
    String path;
    Toast processToast = null;
                ...
                public void onImageAvailable(ImageReader reader) {
                    try {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(getApplicationContext(), "ระบบมีการทำงานที่มากเกินไป กรุณารอสักครู่ แล้วตรวจใหม่อีกครั้ง", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                }
        ...
        public void run() {
            ...
            new AsyncTask<Void, Void, Integer>() {
                ...
                protected Integer doInBackground(Void... voids) {
                    return preprocessing(mImageFileName);
                }
                ...
                protected void onPostExecute(Integer integer) {
                    if (integer > 0 && integer < 500) {
                        ...
                        processToast.cancel();
                        Toast.makeText(getApplicationContext(), "ไม่สามารถอัพโหลดข้อมูลได้ กรุณาตรวจสอบการเชื่อมต่ออินเทอร์เน็ต (error :" + integer + ")", Toast.LENGTH_SHORT).show();
                    }
                }
            }.execute();
        }
    ...
    private int preprocessing(String filePath) {
        Log.i("photopath", filePath);
        File sourceFile = new File(filePath);
        int response = 0;
        Log.i("File", "Value:" + (sourceFile != null ? sourceFile.getPath() : null));
        if (!sourceFile.isFile()) {
            Log.e("CheckFile", "Source File not exist :" + filePath);
        } else {
            Log.i("CheckFile", "Found : " + sourceFile.getAbsolutePath());
            File image = new File(mImageFileName);
            BitmapFactory.Options bmOptions = new BitmapFactory.Options();
            Bitmap bitmap = BitmapFactory.decodeFile(image.getAbsolutePath(), bmOptions);
            Bitmap b = Bitmap.createScaledBitmap(bitmap, bitmap.getWidth() / 2, bitmap.getHeight() / 2, false);
            System.out.println(b.getWidth() + "*" + bitmap.getHeight());
            Bitmap resultBitmap = null;
            try {
                Bitmap temp = ImagePossessing.toGrayScale(b);
                Bitmap temp1 = ImagePossessing.totBlackWrite(temp);
                resultBitmap = ImagePossessing.cutBackGroundImage(temp1, b);
            } catch (OutOfMemoryError e) {
                Toast.makeText(getApplicationContext(), "OOM Error", Toast.LENGTH_SHORT).show();
                Log.e("ImageProcessing", e.toString());
            } catch (Exception e) {
                Log.e("Other Error", e.toString());
            }
            File file = dumpImage(resultBitmap);
            if (file != null) {
    ...
    @Nullable
    private File dumpImage(Bitmap bitmap) {
        if (bitmap != null) {
    ...
    int uploadFile(File file) {
        ...
        path = examStorage.getExamStoragePath();
        if (file.isFile()) {
            MediaType mediaType_jpg = MediaType.parse("image/jpg");
            RequestBody request = new MultipartBody.Builder()
                    ...
                    .addFormDataPart("uploaded_file", file.getName(), RequestBody.create(mediaType_jpg, file)).build();
            response = ConnectServer.connectHttp(url, request);
    ...
    private CameraCaptureSession.CaptureCallback mPreviewCaptureCallback = new
            CameraCaptureSession.CaptureCallback() {
                    ...
                    switch (mCaptureState) {
                        ...
                        case STATE_WAIT_LOCK:
                            ...
                            if (afState == CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED ||
                                ...
                                Toast.makeText(getApplicationContext(), "กำลังประมวลผล...", Toast.LENGTH_SHORT).show();
                ...
                public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request, TotalCaptureResult result) {
                    ...
                    process(result);
                    Log.i("onCaptureCompleted", "Complete");
                }
            };
